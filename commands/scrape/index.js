...
const { handleCreateFolder, readFile, writeFile, getOutputFilePath,
  getInstagramGISHash, getProfileMediaVariables, httpHeaders,
  prettyPrintJson, sleep, parseJson, randomInt
} = require('../../utils');
const apiErrorTransform = require('../../utils/apiErrorTransform');

const {
  USER_AGENT, BASE_URL, NUM_TO_CALC_AVERAGE_ENGAGEMENT,
  MAX_MEDIA_PER_SCRAPE, PROFILE_MEDIA_QUERY_HASH, GRAPHQL_URL, POST_URL, VIRAL_THRESHOLD,
  NUM_TO_SCRAPE
} = require('../../constants');
...
async function _scrapeUsers() {
  ...
  try {
    ...
    if (userViral.isPrivate) {
      console.log(`${userViral.username} is private. Skipping...`);
      return;
    }
    if (userViral.numPosts < NUM_TO_CALC_AVERAGE_ENGAGEMENT) {
      console.log(`${userViral.username} has ${userViral.numPosts} posts, which is fewer than the required ${NUM_TO_CALC_AVERAGE_ENGAGEMENT} posts. Skipping...`);
      return;
    }
      ...
      async ontext(text) {
        ...
        if (text.substring(0, textToScan.length) === textToScan) {
          ...
          if (match) {
            ...
            await writeFile('user_shared_data_sample.txt', prettyPrintJson(json));
            // User public account info
            await writeFile('user_web_data_sample.txt', prettyPrintJson(userViral.userWebData));
            resolve();
          } else {
...
function getSinglePostByType(isVideo = false) {
  return userViral.posts.find(post => isVideo === post.is_video);
}
...
async function getProfileMedia(numMedia, endCursor) {
  ...
  const url = `${BASE_URL}/${GRAPHQL_URL}query_hash=${PROFILE_MEDIA_QUERY_HASH}&variables=${encodeURIComponent(queryVariables)}`;
  const config = {
...
async function downloadPosts() {
  ...
  let result = await getProfileMedia(MAX_MEDIA_PER_SCRAPE, '');
  ...
  while (pageInfo.has_next_page && userViral.currScrapeCount < NUM_TO_SCRAPE) {
    ...
    result = await getProfileMedia(MAX_MEDIA_PER_SCRAPE, pageInfo.end_cursor);
  ...
  writeFile('profile_media_sample.txt', prettyPrintJson(result));
  writeFile('image_post_sample.txt', prettyPrintJson(getSinglePostByType()));
  writeFile('video_post_sample.txt', prettyPrintJson(getSinglePostByType(true)));
}
...
async function saveViralContent() {
  await writeFile('viral_posts_sample.txt', prettyPrintJson(userViral.viralPosts));
  ...
  const filename = getOutputFilePath(userViral.numFollowers, userViral.averageLikes,
    userViral.averageComments);
