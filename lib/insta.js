...
const {
  NUM_TO_SCRAPE, PROFILE_MEDIA_QUERY_HASH, GRAPHQL_URL, MAX_MEDIA_PER_SCRAPE,
...
async function downloadPosts(id, rhxGis, username) {
  ...
  let result = await getProfileMedia(MAX_MEDIA_PER_SCRAPE, id, username, rhxGis);
  ...
  while (pageInfo.has_next_page && scrapeCount < NUM_TO_SCRAPE) {
    ...
    result = await getProfileMedia(MAX_MEDIA_PER_SCRAPE, id, username, rhxGis, pageInfo.end_cursor);
